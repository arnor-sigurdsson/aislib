...
def test_iter_loadtxt(tmp_path):
    ...
    loaded_arr_skipped = loader(arr_path,
                                delimiter=',',
                                skiprows=5,
                                dtype=float)
    ...
    loaded_arr_custom = loader(arr_path,
                               custom_splitter=lambda x: x.strip().split(','))
    np.testing.assert_array_equal(test_arr, loaded_arr_custom)


@pytest.mark.parametrize('create_test_arr_files',
                         (np.packbits,),
                         indirect=True)
def test_load_np_packbits_from_folder(create_test_arr_files):
    ...
    loaded_arrs, loaded_ids = loader(arr_path,
                                     4,
                                     dtype=np.uint8)
