...
def calc_size_after_conv_sequence(input_width: int,
    ...
    def calc_output_size(size, layer):
        return (size - layer.kernel_size[axis] + 2 * layer.padding[axis]) / \
               layer.stride[axis] + 1
    ...
    for block in conv_sequence:
        ...
        conv_operations = [i for i in vars(block)['_modules']
                           if i.find('conv') != -1]
    ...
    if int(current_size) == 0:
        raise ValueError('Calculated size after convolution sequence is 0,'
                         'check the number of convolutions and their params.')

    return int(round(current_size))


def calc_conv_padding_needed(input_width: int,
                             kernel_size: int,
                             stride: int):

    if [i for i in locals().values() if i < 0]:
        raise ValueError(f'Got negative value when expected positive in the'
